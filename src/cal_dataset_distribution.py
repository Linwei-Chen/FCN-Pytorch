...
minh:71, minw:142, meanh:386.31915455746366, meanw:470.4435931307794

SBD 数据集的BGR平均值为:
[103.1833754149088, 111.68176112561423, 116.77091541098135]
...
cityscape 数据集尺寸平均值
minh:1024, minw:2048, meanh:1024.0, meanw:2048.0
数据集的BGR平均值为:
[72.39279085257427, 82.90903173265797, 73.15920695684545]
数据集的BGR标准差为:
[10.360337038857121, 10.71265687582664, 11.849564804734342]
"""


def cal_imgs_mean(imgs_path):
    # ims_list = os.listdir(imgs_path)
    ims_list = from_dir_get_imgs_list(imgs_path)
    B_means = []
    G_means = []
    R_means = []
...
if __name__ == '__main__':
    ...
    sbd = '/Users/chenlinwei/dataset/SBD_FULL11356/dataset/img'
    # cal_imgs_size_mean(imgs_path=sbd)
    # cal_imgs_mean(imgs_path=sbd)
    cal_imgs_std(imgs_path=sbd, BGR_mean=[103.1833754149088, 111.68176112561423, 116.77091541098135])

    cityscape = '/Users/chenlinwei/dataset/cityscapes/leftImg8bit/train/'
    # cal_imgs_size_mean(imgs_path=cityscape)
    # cal_imgs_mean(imgs_path=cityscape)
    # cal_imgs_std(imgs_path=cityscape, BGR_mean=[72.39279085257427, 82.90903173265797, 73.15920695684545])
