...
"""
cityscape_RGB_mean = [73.15920695684545, 82.90903173265797, 72.39279085257427]


def get_cityscape_data_loader(args):
    cityscape_transform = Compose2([RandomCrop(size=args.crop_size, pad_if_needed=True),
                                    RandomHorizontalFlip(p=0.5), PhotometricDistort(),
                                    ToTensor(), Normalize(cityscape_RGB_mean, [1.00, 1.00, 1.00])])

    cityscape_dataset = Cityscapes(root=args.cityscape_data_path,
                                   split='train', mode='fine', target_type='semantic',
                                   transform=None, target_transform=None,
                                   transforms=cityscape_transform)

    return DataLoader(dataset=cityscape_dataset,
...
if __name__ == '__main__':
    ...
    args =config()
    cityscape_dataset = get_cityscape_data_loader(args)
    for _, (imgs, targets) in enumerate(cityscape_dataset):
        print(imgs, targets)
    pass
