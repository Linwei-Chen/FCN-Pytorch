...
def get_model(name: str, n_class=21):
    if name.lower() == 'fcn32s':
        return FCN32s(pretrained_net=VGGNet(), n_class=n_class)
    elif name.lower() == 'fcn16s':
        return FCN16s(pretrained_net=VGGNet(), n_class=n_class)
    elif name.lower() == 'fcn8s':
        return FCN8s(pretrained_net=VGGNet(), n_class=n_class)
    elif name.lower() == 'fcns':
        return FCNs(pretrained_net=VGGNet(), n_class=n_class)
    else:
        raise Exception(f'*** model name wrong, {name} not legal, '
                        f'please choose from fcn32s, fcn16s, fcn8s, fcns')
...
if __name__ == "__main__":
    ...
    optimizer = optim.SGD(fcn_model.parameters(), lr=1e-1, momentum=0.9)
    input = torch.randn(batch_size, 3, h, w)
